frappe.ui.form.on("NBR VAT Report", {
	...
	refresh: function (frm) {
		...
		frm.page.set_secondary_action(__("Download Excel"), function () {
			...
			let url = `/api/method/beveren_health.beveren_health.utils.nbr_vat_api.download_vat_excel?company=${encodeURIComponent(
				company
			)}&from_date=${from_date}&to_date=${to_date}&tax_id=${encodeURIComponent(tax_id)}`;
		...
		if (!frm.doc.company || !frm.doc.from_date || !frm.doc.to_date) {
			frm.set_df_property(
				...
				'<div class="alert alert-info">' +
					__("Please select Company, From Date, and To Date to generate the report.") +
					"</div>"
			);
			return;
		}
	...
	render_report: function (frm) {
		...
		html += `
			<div class="vat-summary-row">
				...
				<div class="vat-summary-item" style="border-bottom: 3px solid ${
					net_vat_due >= 0 ? "#48bb78" : "#e53e3e"
				};">
		...
		let summary_active =
			frm.report_state.active_view === "Summary" ? "active btn-info" : "btn-default";
		let detailed_active =
			frm.report_state.active_view === "Detailed" ? "active btn-info" : "btn-default";
				...
				<div>
					${
						frm.report_state.active_view === "Summary"
							? frm.events.get_breadcrumbs_html(frm)
							: '<span class="vat-breadcrumb-item active">' +
							  __("All Contributing Transactions (Detailed View)") +
							  "</span>"
					}
				</div>
				<div class="btn-group" role="group">
					<button type="button" class="btn btn-xs btn-summary-toggle ${summary_active}">${__(
			"Summary"
		)}</button>
					<button type="button" class="btn btn-xs btn-detailed-toggle ${detailed_active}">${__(
			"Detailed"
		)}</button>
				</div>
			</div>
	...
	get_breadcrumbs_html: function (frm) {
		...
		if (frm.report_state.level === 1) {
			html +=
				'<span class="vat-breadcrumb-item active">' + __("Summary of Books") + "</span>";
		} else {
			html +=
				'<a class="vat-breadcrumb-item btn-back-level1">' +
				__("Summary of Books") +
				"</a>";
	...
	get_form_html: function (frm) {
		...
		html += `
			<tr class="parent-node ${sales_class}" data-group="sales">
				...
				<td style="font-weight: bold;"><i class="fa fa-chevron-down"></i> ${__(
					"VAT on Sales (Output VAT)"
				)}</td>
				<td class="vat-amount-col" style="font-weight: bold;">${frm.events.format_count(
					rows["7"].count
				)}</td>
				<td class="vat-amount-col" style="font-weight: bold;">${frm.events.format_bhd(
					rows["7"].amount
				)}</td>
				<td class="vat-amount-col" style="font-weight: bold;">${frm.events.format_bhd(
					rows["7"].adjustment
		...
		html += `
			<tr class="parent-node ${net_class}" data-group="net">
				...
				<td class="vat-amount-col" style="font-weight: bold;">-</td>
				<td class="vat-amount-col" style="font-weight: bold;">-</td>
				<td class="vat-amount-col" style="font-weight: bold; color: ${
					net_vat_due >= 0 ? "#2f855a" : "#c53030"
				}">
					${frm.events.format_bhd(net_vat_due)}
				</td>
			</tr>
		...
		html += `
			...
			<tr class="child-node" data-parent-group="net" ${net_style}>
				...
				<td class="vat-amount-col">-</td>
				<td class="vat-amount-col">-</td>
				<td class="vat-amount-col">${frm.events.format_bhd(
					frm.summary_data.row_17_credit_carried_forward
				)}</td>
			</tr>
			<tr class="child-node total-row" data-parent-group="net" ${net_style} style="background-color: #ebf8ff; color: #2b6cb0;">
				...
				<td class="vat-amount-col" style="font-weight: bold;">-</td>
				<td class="vat-amount-col" style="font-weight: bold;">-</td>
				<td class="vat-amount-col" style="font-weight: bold; font-size: 1.05rem;">${frm.events.format_bhd(
					net_vat_due
				)}</td>
			</tr>
		`;
		...
		html += `
			<div class="vat-footer-toolbar">
				<div class="vat-footer-buttons">
					<button class="btn btn-default btn-xs btn-expand-all-tree"><i class="fa fa-caret-down"></i> ${__(
						"Expand All"
					)}</button>
					<button class="btn btn-default btn-xs btn-collapse-all-tree"><i class="fa fa-caret-right"></i> ${__(
						"Collapse All"
					)}</button>
				</div>
	...
	get_category_html: function (frm) {
		...
		if (["1a", "1b", "2", "3", "4", "5", "6", "8a", "8b", "12", "13"].indexOf(row_id) !== -1) {
			...
			html +=
				`<td><span class="vat-indicator-pill b2b">B2B</span> ` +
				__("Registered / Business Transactions") +
				`</td>`;
			...
			html +=
				`<td><span class="vat-indicator-pill b2c">B2C</span> ` +
				__("Unregistered / Consumer Transactions") +
				`</td>`;
	...
	get_invoices_html: function (frm) {
		...
		if (list.length === 0) {
			...
			list.forEach((inv) => {
				html += "<tr>";
				html += `<td><a href="/app/${frappe.router.slug(inv.doctype)}/${inv.invoice_no}">${
					inv.invoice_no
				}</a></td>`;
				html += `<td>${inv.party}</td>`;
				html += `<td>${frappe.datetime.str_to_user(inv.date)}</td>`;
				html += `<td class="vat-amount-col">${frm.events.format_bhd(
					inv.taxable_value
				)}</td>`;
	...
	get_detailed_list_html: function (frm) {
		...
		html += `
			<div class="vat-section-title-row">
				<div class="vat-section-title"><span class="vat-breadcrumb-item active">${__(
					"All Contributing Transactions (Detailed View)"
				)}</span></div>
			</div>
		`;
