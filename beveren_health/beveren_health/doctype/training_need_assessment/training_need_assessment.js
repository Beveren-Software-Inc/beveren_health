...
frappe.ui.form.on("Training Need Assessment", {
	onload(frm) {
		...
		frm.fields_dict["assessments"].grid.get_field("title").get_query = function (
			doc,
			cdt,
			cdn
		) {
			const selected_title = frm.doc.assessments.map((r) => r.title).filter((e) => e);
			return {
				filters: [["name", "not in", selected_title]],
			};
		};
	},
	refresh(frm) {
