...
def parse_gs1(raw_code):
    ...
    # Strip FNC1 / GS separators
    for ch in [chr(29), '\x1d', '\x1c', '\x1e', '\x1f']:
        raw_code = raw_code.replace(ch, '')

    frappe.logger().info(f"Cleaned GS1 Code: {raw_code}")
    ...
    while i < len(raw_code):
        ...
        if ai == '01':                          # GTIN – fixed 14 digits
            if len(raw_code) >= i + 14:
                result['gtin'] = raw_code[i:i+14]
                i += 14

        elif ai == '11':                        # MFG date – fixed 6 digits
            if len(raw_code) >= i + 6:
                result['mfg_date'] = _parse_date(raw_code[i:i+6])
                i += 6

        elif ai == '17':                        # EXP date – fixed 6 digits
            if len(raw_code) >= i + 6:
                result['expiry_date'] = _parse_date(raw_code[i:i+6])
                i += 6

        elif ai == '10':                        # Batch/Lot – variable length
            value, i = _read_variable(raw_code, i)
            result['batch_no'] = value

        elif ai == '21':                        # Serial – variable length
            value, i = _read_variable(raw_code, i)
            result['serial_no'] = value

        else:
            frappe.logger().warning(f"Unknown GS1 AI: {ai}")
            # Skip 1 char to avoid infinite loop on unknown AIs
            i -= 1

    frappe.logger().info(f"GS1 parsed: {result}")
    return result
