...
DOCTYPE_CONFIG = {
	...
	"Stock Scanner": {
		...
		"additional_fields": {"current_qty": "qty", "current_amount": "amount"},
	},
}
...
def _buying_rate(item_code):
	return frappe.db.get_value("Item Price", {"item_code": item_code, "buying": 1}, "price_list_rate") or 0
...
def process_batch_scan(
	...
	try:
		...
		if doctype not in DOCTYPE_CONFIG:
			return {"success": False, "message": f"Unsupported document type: {doctype}"}
		...
		batch_doc = get_or_create_batch(item_code, parsed["batch_no"], parsed.get("expiry_date"))
		...
		if existing_row_info:
			...
			if existing_row_info["index"] == current_idx:
				...
				if fast_persist:
					result = append_dispensing_lot_to_row(doc, config, existing_row_info["row"], parsed, item)
...
def assign_to_current_row(config, item, parsed, warehouse=None, row_name=None, persist=False, doc=None):
...
def create_new_row(config, item, parsed, warehouse=None, doc=None):
	...
	if doc is not None:
		...
		if config["child_doctype"] == "Purchase Receipt Item":
			_apply_purchase_receipt_qty_fields(row_data, qty, row=None, child_doctype=config["child_doctype"])
...
def parse_gs1(raw_code):
	...
	frappe.logger().info(f"Raw GS1 Code: {raw_code!r}")
...
def get_or_create_batch(item_code, batch_no, expiry_date=None):
	...
	existing_batch = frappe.db.get_value("Batch", {"batch_id": batch_no}, ["name", "item"], as_dict=True)
