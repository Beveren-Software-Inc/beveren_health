...
def _validate_pack_sale_on_lot(lot, item_code, issue_uom, issue_qty):
	...
	if flt(issue_qty) > 1:
		frappe.throw(
			_("Only one {0} can be sold per Dispensing Lot on a single line").format(stock_uom)
		)
...
def _resolve_dispensing_lot_from_si_row(item_row):
	"""Return Dispensing Lot name from SI item row (Link field or manufacturer serial)."""
	lot_name = item_row.get("custom_dispensing_lot")
	if lot_name and frappe.db.exists("Dispensing Lot", lot_name):
		return lot_name

	if lot_name:
		by_serial = frappe.db.get_value("Dispensing Lot", {"serial_no": lot_name}, "name")
		if by_serial:
			return by_serial

	serial_no = (item_row.get("serial_no") or "").strip()
	if serial_no:
		first_serial = serial_no.split("\n")[0].split(",")[0].strip()
		if first_serial:
			return frappe.db.get_value("Dispensing Lot", {"serial_no": first_serial}, "name")
	return None


def apply_sales_invoice_to_dispensing_lot(item_row, reference_doctype, reference_name, posting_date, is_return=False):
	"""Post Out (sale) or In (return) on the linked dispensing lot."""
	lot_name = _resolve_dispensing_lot_from_si_row(item_row)
	if not lot_name:
		return
	...
	if not is_return:
		validate_dispensing_lot_for_sale(item_row, lot)
...
def validate_sales_invoice_dispensing_lots(doc):
	...
	for row in doc.items:
		...
		if item_requires_dispensing_lot(row.item_code) and not _resolve_dispensing_lot_from_si_row(row):
			frappe.throw(
		...
		lot_name = _resolve_dispensing_lot_from_si_row(row)
		if not lot_name:
			continue
...
def process_sales_invoice_dispensing_lots(doc, is_return=False):
	for row in doc.items:
		if not _resolve_dispensing_lot_from_si_row(row):
			continue
...
def reverse_sales_invoice_dispensing_lots(doc):
	...
	for row in doc.items:
		...
		lot = frappe.get_doc("Dispensing Lot", lot_name)
		issue_uom, issue_qty = compute_issue_from_sales_item(row, lot)
		if not issue_qty:
			continue
