...
def validate_batch(doc, method=None):
	existing_batch = frappe.db.get_value("Batch", {"batch_id": doc.batch_id}, ["name", "item"], as_dict=True)
	...
	if existing_batch:
		...
		frappe.logger().info(
			f"Batch {doc.batch_id} already exists for item {existing_batch.item}. Creating {unique_batch_id} for {doc.item}"
		)
