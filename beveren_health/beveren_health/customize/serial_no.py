...
def set_gtin_universal(doc, method):
	...
	# Fetch rows
	rows = frappe.get_all(child_doctype, filters={"parent": voucher_no}, fields=["serial_no", "custom_gstin"])
	...
	for row in rows:
		...
		if doc.name in serials:
			frappe.db.set_value("Serial No", doc.name, "custom_gtin", row.custom_gstin)
			return
...
def update_serial_gtin(doc, method):
	...
	child_table_map = {"Purchase Receipt": "items", "Stock Entry": "items", "Stock Reconciliation": "items"}
	...
	for row in doc.get(child_field):
		...
		for serial in serials:
			...
			# Update Serial No
			frappe.db.set_value("Serial No", serial, "custom_gtin", row.custom_gstin)
