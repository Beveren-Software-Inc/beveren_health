...
class OvertimeSlip(BaseOvertimeSlip):
	def before_submit(self):
		...
		if total:
			...
			self.total_overtime_duration = (
				self.custom_total_over_time_duration - self.custom_total_less_time_duration
			)
	...
	def calculate_overtime_amount(
		...
		if overtime_duration <= overtime_details.get("custom_rate_change_cutoff_hours", multiplier):
			...
			amount = (
				overtime_details.get("custom_rate_change_cutoff_hours") * applicable_hourly_rate * multiplier
			)
			amount += (
				(overtime_duration - overtime_details.get("custom_rate_change_cutoff_hours"))
				* applicable_hourly_rate
				* multiplier_after_cutoff_hours
			)
		return amount
