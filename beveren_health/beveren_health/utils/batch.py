...
def batch_before_save(doc, method=None):
	...
	try:
		...
		existing = frappe.db.get_value("Item Barcode", {"parent": doc.item, "custom_batch": doc.name}, "name")
		if existing:
...
def generate_barcode_for_existing_batches():
	...
	batches = frappe.get_all(
		"Batch", filters={"custom_barcode": ["is", "not set"], "docstatus": 0}, pluck="name"
	)
	...
	for batch_name in batches:
		try:
			...
			if not batch.custom_barcode:
				batch.custom_barcode = generate_ean13_barcode()

			# Generate barcode image
			if batch.custom_barcode and not batch.custom_barcode_image:
				batch.custom_barcode_image = generate_barcode_image(
					batch.custom_barcode, DEFAULT_BARCODE_TYPE
				)

			# Save changes
			batch.db_set(
				{"custom_barcode": batch.custom_barcode, "custom_barcode_image": batch.custom_barcode_image}
			)
			...
			frappe.log_error(
				title="Batch Barcode Generation Error", message=f"Error for batch {batch_name}: {e!s}"
			)
...
def regenerate_barcode_image_for_batch(batch_name):
	...
	try:
		...
		if batch.custom_barcode:
			# Generate new image
			image_url = generate_barcode_image(batch.custom_barcode, DEFAULT_BARCODE_TYPE)
		...
		frappe.log_error(
			title="Batch Barcode Regeneration Error", message=f"Error for batch {batch_name}: {e!s}"
		)
		return False, str(e)
