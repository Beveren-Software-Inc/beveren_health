...
def get_item_and_batch_from_barcode(barcode, warehouse=None):
	...
	"""
	result = {"item_code": None, "batch_no": None, "qty": 1}
	...
	item_match = frappe.db.get_value(
		"Item Barcode", {"barcode": barcode}, ["parent", "custom_batch"], as_dict=True
	)
	if item_match:
...
def _with_batch_qty(result, warehouse):
	...
	if result.get("batch_no") and warehouse:
		...
		qty = get_batch_qty(batch_no=result["batch_no"], warehouse=warehouse, item_code=result["item_code"])
