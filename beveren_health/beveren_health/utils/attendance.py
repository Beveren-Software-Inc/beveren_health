	...
	for shift in shift_types:
		try:
			...
			latest_checkin = frappe.db.get_value(
				"Employee Checkin", filters={"shift": shift["name"]}, fieldname="time", order_by="time desc"
			)

			if latest_checkin:
				frappe.db.set_value("Shift Type", shift["name"], "last_sync_of_checkin", latest_checkin)
			else:
