...
frappe.ui.form.on("Item Group", {
	refresh(frm) {
		if (!frm.is_new()) {
					...
					frappe.confirm(
						...
						function () {
							frappe.call({
								...
								error: function (r) {
									frappe.msgprint(
										__("Error: {0}", [r.message || "Unknown error"])
									);
								},
							});
						},
			...
			frm.add_custom_button(
				...
				function () {
					frappe.confirm(
						...
						function () {
							frappe.call({
								method: "beveren_health.beveren_health.customize.item_group.clear_has_serial_no_for_item_group",
								args: { item_group: frm.doc.name },
								callback: function (r) {
									if (r.message && r.message.queued) {
										frappe.show_alert({
			...
			frm.add_custom_button(
				__("Migrate Serials to Dispensing Lot"),
				function () {
					frappe.confirm(
						__(
						...
						function () {
							frappe.call({
								method: "beveren_health.beveren_health.customize.item_group.migrate_serials_to_dispensing_lots",
								args: { item_group: frm.doc.name },
								callback: function (r) {
									if (r.message && r.message.queued) {
										frappe.show_alert({
			...
			frm.add_custom_button(
				__("Enable Has Dispense Lot from Lots"),
				function () {
					frappe.confirm(
						__(
						...
						function () {
							frappe.call({
								method: "beveren_health.beveren_health.customize.item_group.flag_has_dispense_lot_from_dispensing_lots",
								args: { item_group: frm.doc.name },
								callback: function (r) {
									if (r.message && r.message.queued) {
										frappe.show_alert({
			...
			frm.add_custom_button(
				...
				function () {
					frappe.confirm(
						...
						function () {
							frappe.call({
								method: "beveren_health.beveren_health.customize.item_group.clear_has_dispense_lot_for_item_group",
								args: { item_group: frm.doc.name },
								callback: function (r) {
									if (r.message && r.message.queued) {
										frappe.show_alert({
...
frappe.realtime.on("item_group_clear_serial_no_done", function (data) {
	const msg =
		data.message ||
		(data.error ? __("Clear Has Serial No failed.") : __("Clear Has Serial No completed."));
	frappe.show_alert({
		message: msg,
		indicator: data.error ? "red" : "green",
	});
