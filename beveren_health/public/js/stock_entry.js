...
function update_batch_dates_from_stock_entry(frm) {
	...
	frappe.confirm(
		...
		() => {
			frappe.call({
				method: "beveren_health.beveren_health.override.batch.update_batch_dates_from_stock_entry",
...
function get_warehouse_for_stock_entry(frm, row) {
	...
	switch (purpose) {
		...
		default:
			return doc.to_warehouse || doc.from_warehouse || doc.t_warehouse || doc.s_warehouse;
	}
}
...
function handle_assign_to_current(frm, cdt, cdn, result, row_idx, warehouse) {
	if (result.server_persisted) {
		...
		if (purpose === "Material Receipt" || purpose === "Manufacture" || purpose === "Repack") {
			wh_vals.t_warehouse = warehouse;
		} else if (
			purpose === "Material Issue" ||
			purpose === "Material Transfer for Manufacture"
		) {
			wh_vals.s_warehouse = warehouse;
		} else if (purpose === "Material Transfer") {
			wh_vals.s_warehouse = frm.doc.from_warehouse || frm.doc.s_warehouse;
			wh_vals.t_warehouse = frm.doc.to_warehouse || frm.doc.t_warehouse || warehouse;
		} else if (warehouse) {
			wh_vals.t_warehouse = warehouse;
		}
...
function set_warehouse_for_row(frm, cdt, cdn, warehouse) {
	...
	switch (purpose) {
		...
		case "Material Transfer for Manufacture":
			...
			break;
		case "Material Transfer":
			frappe.model.set_value(cdt, cdn, "s_warehouse", doc.from_warehouse || doc.s_warehouse);
...
function handle_append_serial(frm, cdt, cdn, result, row_idx) {
	if (result.server_persisted) {
		beveren_health.auto_save_scan.patch_row(cdt, cdn, {
			qty: result.new_qty,
			transfer_qty: result.new_qty,
			amount: result.new_amount,
			basic_amount: result.new_amount,
	...
	if (target_row) {
		...
		if (result.server_persisted) {
			beveren_health.auto_save_scan.patch_row(cdt, cdn, {
				...
				transfer_qty: result.new_qty != null ? result.new_qty : target_row.transfer_qty,
				amount: result.new_amount != null ? result.new_amount : target_row.amount,
				basic_amount:
					result.new_amount != null ? result.new_amount : target_row.basic_amount,
...
function render_se_labels(print_rows, batch_data_map, branch_display) {
	...
	print_rows.forEach((row) => {
		...
		for (let i = 0; i < row.print_qty; i++) {
			labels_html.push(
				'<div class="label-page">' + build_se_label_html(data, branch_display) + "</div>"
			);
		}
	});
